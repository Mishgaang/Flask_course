...
class TestFilms:
    ...
    @patch('Lesson2.src.services.film_service.FilmService.fetch_all_films', autospec=True)
    ...
    def test_create_film_with_mock_db(self):
        with patch('Lesson2.src.db.session.add', autospec=True) as mock_session_add, \
                patch('Lesson2.src.db.session.commit', autospec=True) as mock_session_commit:
            client = app.test_client()
            data = {
                'title': 'Test Title',
                'distributed_by': 'Test Company',
    ...
    def test_update_film_with_mock_db(self):
        with patch('Lesson2.src.services.film_service.FilmService.fetch_film_by_uuid') as mocked_query, \
                patch('Lesson2.src.db.session.add', autospec=True) as mock_session_add, \
                patch('Lesson2.src.db.session.commit', autospec=True) as mock_session_commit:
